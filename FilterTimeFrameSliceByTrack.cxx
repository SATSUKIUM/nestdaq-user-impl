...
*/
      ...
      if(DetectorName == "kldc"){
         ...
         std::cout << "\t" << DetectorName << "(index: " << static_cast<uint8_t>(DetectorNameIdx) << "), " << PlaneName << "(index: " << static_cast<uint8_t>(PlaneNameIdx) << "), Segment: " << static_cast<uint8_t>(SegmentNumber) << std::endl;
         for(int i=0+32; i<128-32; ++i){
            int ChannelNumber = i;
            uint32_t dopeKey_DETtoFE;
            #if 1
            std::cout << funcname << "channel map query: " << std::endl;
            std::cout << "\t" << DetectorName << "(index: " << static_cast<uint8_t>(DetectorNameIdx) << "), " << PlaneName << "(index: " << static_cast<uint8_t>(PlaneNameIdx) << "), Segment: " << static_cast<uint8_t>(SegmentNumber) << ", ChannelName: " << ChannelName << "(index: " << static_cast<uint8_t>(ChannelNameIdx) << "), ChannelNumber: " << static_cast<uint16_t>(ChannelNumber) << std::endl;
            #endif
            bool found_DETtoFE = fChMap->getDopeKey_DETtoFE(DetectorNameIdx, PlaneNameIdx, static_cast<uint8_t>(SegmentNumber), ChannelNameIdx, static_cast<uint16_t>(ChannelNumber), dopeKey_DETtoFE);
            if(found_DETtoFE){
...
bool FilterTimeFrameSliceByTrack::RegisterDetectorConfig_DCTdcCalib()
{
   ...
   for(const auto& calib : fTemporaryDCTdcCalibs){
      ...
      if(isFirstEntry){
         ...
         std::cout << "\t" << DetectorName << ", " << PlaneName << ", Segment: " << static_cast<uint8_t>(SegmentNumber) << ",wire: " << wireId << std::endl;
         isFirstEntry = false;
      }
...
*/
      ...
      if(DetectorName == "kldc"){
         ...
         uint32_t dopeKey_DETtoFE;
         bool found_DETtoFE = fChMap->getDopeKey_DETtoFE(DetectorName, PlaneName, static_cast<uint8_t>(SegmentNumber), ChannelName, static_cast<uint16_t>(ChannelNumber), dopeKey_DETtoFE);
         if(found_DETtoFE){
            chmap::FEAddrItem feaddritem = fChMap->getFEAddrItem(dopeKey_DETtoFE);
            uint32_t dopeKeyFEtoDET;
            bool found_FEtoDET = fChMap->getDopeKey_FEtoDET(feaddritem.ip3rd, feaddritem.ip4th, feaddritem.ch, dopeKeyFEtoDET);
